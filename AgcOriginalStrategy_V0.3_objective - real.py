...
class AgcMaster:
    def __init__(self, p_limit, agc_obj_dict):
        ...
        for key in agc_obj_dict:
            ...
            self.dic_catchup_rec[key] = list([1])
            self.dic_catch_mode_rec[key] = list([1])

        for key in self.AgcObjDict:
            self.TotalCapacity += self.AgcObjDict[key].d_capacity
        for key in self.AgcObjDict:
            self.AgcObjDict[key].d_p_base = self.AgcObjDict[key].d_capacity / self.TotalCapacity
    ...
    def AgcCatchModeRealOutSumUp(self, which_mode_to_sum_up, t_step):
        ...
        ret = np.double(0)
        for key in self.AgcObjDict:
            if self.CatchUpMode(self.AgcObjDict[key].d_real_out, self.dic_catch_mode_rec[key][t_step]) == which_mode_to_sum_up:
                ret += self.AgcObjDict[key].d_real_out
            else:
                pass
        return ret
    ...
    def DistributeRef(self, dic_p_ref_previous):
        dic_tmp_p_ref = dict()
        # Case 1: < LIMIT
        if self.AgcRealOutSumUp() < 0.98 * self.P_LIMIT:
            ...
            # Case 1.1: <= P_base
                if self.AgcObjDict[key].d_real_out < self.AgcObjDict[key].d_p_base:
                    dic_tmp_p_ref[key] = self.AgcObjDict[key].d_real_out + self.AgcObjDict[key].d_p_delta
            # Case 1.2: > P_base
                else:
                    catch_mode = self.CatchUpCheck(self.AgcObjDict[key].d_real_out, dic_p_ref_previous[key])
    ...
    def MainLoop(self):
        ...
        # dic_tmp_catch_mode = dict()

        for key in self.AgcObjDict:
            dic_tmp_p_ref[key] = 0
            # dic_tmp_catch_mode[key] = 0
...
#     SiteTsDict[name] = SiteFile[name]['出力值'].tolist()
